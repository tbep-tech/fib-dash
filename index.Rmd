...
entmatrix <- reactive({
  ...
  stas <- enterodata %>%
    dplyr::filter(long_name %in% areasel1) %>%
    dplyr::select(station) %>%
    dplyr::distinct() %>%
    dplyr::pull()

  out <- try(tbeptools::show_fibmatrix(enterodata, stas = stas, yrrng = c(NA, maxyr), indic = 'ecocci', plotly = T))
  
  validate(
    need(!inherits(out, 'try-error'), 'No enterococcus data available for selection')
  )
...
enterotomap <- reactive({
  ...
  mosel1 <- lubridate::month(mosel1)

  tomap <- isolate(enterotomap_fun(enterodata, yrsel1, mosel1, areasel1, wetdry = TRUE, precipdata = catchprecip, temporal_window = 2, wet_threshold = 0.5))
...
fibmatrix <- reactive({
  ...
  out <- try(tbeptools::show_fibmatrix(fibdata, stas = stas, yrrng = c(NA, maxyr), indic = 'fcolif', plotly = T))
  
  validate(
    need(!inherits(out, 'try-error'), 'No fecal coliform data available for selection')
  )
...
fibtomap <- reactive({
  ...
  mosel2 <- as.integer(lubridate::month(mosel2))
  
  tomap <- isolate(fibtomap_fun(fibdata, yrsel2, mosel2, areasel2))
...
output$entmap <- leaflet::renderLeaflet({
  ...
  tbeptools::show_enteromap(enterodata, yrsel = maxyr, mosel = 7, areasel = c('Hillsborough Bay', 'Old Tampa Bay'),
...
fillCol(flex = c(NA, 1),
  column(12,
    ...
    column(4, 
      sliderInput('mosel1', 'Select month:', min = as.Date(paste0(maxyr, '-01-01')), max = as.Date(paste0(maxyr, '-12-01')), value = as.Date(paste0(maxyr, '-07-01')), timeFormat = "%b")
    ),
    column(4,
      selectInput('areasel1', 'Select area:', choices = areas1, selected = c('Hillsborough Bay', 'Old Tampa Bay'), width = '200%', multiple = T)
    )
  ),
...
### *ENTEROCOCCUS* AND *E. COLI* MAP SUMMARY
...
fillCol(flex = c(NA, 1),
  column(12,
    ...
    column(4, 
      sliderInput('mosel2', 'Select month:', min = as.Date(paste0(maxyr, '-01-01')),max = as.Date(paste0(maxyr, '-12-01')), value = as.Date(paste0(maxyr, '-07-01')), timeFormat = "%b")
    ),
    column(4,
