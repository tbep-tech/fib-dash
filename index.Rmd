...
```{r output}
# # epc map year selection
# output$renderyrsel2 <- renderUI({
#         
#   # input
#   areasel2 <- input$areasel2
#   
#   minyr <- fibdata |> 
#     dplyr::filter(area %in% areasel2) |> 
#     dplyr::filter(!(is.na(ecoli) & is.na(ecocci))) |> 
#     dplyr::pull(yr) |>
#     min() 
#   
#   sliderInput('yrsel2', 'Select year:', min = minyr, max = maxyr, value = maxyr, step = 1, sep = '', width = '200%')
#     
# })
# 
# # epc map month selection
# output$rendermosel2 <- renderUI({
#   
#   # input
#   yrsel2 <- input$yrsel2
#   areasel2 <- input$areasel2
...
#     dplyr::filter(area %in% areasel2) |> 
#     dplyr::filter(!(is.na(ecoli) & is.na(ecocci))) |> 
#     dplyr::filter(yr == yrsel2) |> 
#     dplyr::pull(mo) |> 
#     unique()
# 
#   mosel <- mos[mos %in% mosel]
#   
#   selectInput('mosel2', 'Select month:', choices = mosel, selected = mosel[1], width = '200%')
#   
# })
